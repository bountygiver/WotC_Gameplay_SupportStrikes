...
//---------------------------------------------------------------------------------------
class X2Condition_CS_UnitType extends X2Condition;
...
var() bool bUnitTemplateNameInsteadOfCharacterGroupName;

var() bool bCheckSource;	// This will allow the condition to be checked via CanEverBeValid instead of the normal conditions
...
function name MeetsCondition(XComGameState_BaseObject kTarget)
{
	...
	if (UnitState != none)
	{
		if (bUnitTemplateNameInsteadOfCharacterGroupName)
		{
			UnitTypeName = UnitState.GetMyTemplateName();
		}
		else
		{
...
{
	if ( bCheckSource && MeetsCondition(SourceUnit) == 'AA_Success' )
		return true;
